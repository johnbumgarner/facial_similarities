...
def prepare_training_data(images, image_height, image_width):
    ...
    for image in images:
        ...
        if not name in image_names:
            ...
            # cv2.cvtColor(input_image, flag) where flag determines the type of conversion
            flipped_grayscale_image = cv2.cvtColor(horizontal_flip, cv2.COLOR_BGR2GRAY)

            # faces contains the calculate facial coordinates produced by face_cascade.detectMultiScale
            faces = face_cascade.detectMultiScale(grayscale_image, scaleFactor = 1.5, minNeighbors = 5, flags = cv2.CASCADE_SCALE_IMAGE)

            # faces contains the calculate facial coordinates produced by face_cascade.detectMultiScale
            flipped_faces = face_cascade.detectMultiScale(flipped_grayscale_image, scaleFactor=1.5, minNeighbors=5, flags=cv2.CASCADE_SCALE_IMAGE)
            ...
            for (x_coordinate, y_coordinate, width, height) in flipped_faces:
                current_id += 1
                current_name = (f'{name}_flipped')
                image_names[current_name] = current_id
                id_ = image_names[current_name]

                print(f'Processing training data for image: {name}')

                # roi_gray is a numpy.ndarray based on the gray scale of the image
                roi_gray = flipped_grayscale_image[y_coordinate:y_coordinate + height, x_coordinate:x_coordinate + width]
...
def facial_recognition(image_name, image_height, image_width, training_data, image_labels):
    '''
...
training_data = 'face_train_data.yml'
...
if initial_training_data:
    ...
    ###########################################################
    image_name = 'flip_jennifer_aniston.jpeg'
    image_prediction = facial_recognition(image_name, image_height, image_width, training_data, image_labels)
